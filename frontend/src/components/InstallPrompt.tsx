...
import { Smartphone, Monitor, ChevronRight, Download, Gift, Star, Zap, Bell, X } from 'lucide-react';
    ...
    <>
     ...
     {!isVisible && !installedSuccessfully && !isMinimized && !deferredPrompt && isIOS && (
          <motion.div 
           initial={{ y: 100 }}
           animate={{ y: 0 }}
           className="fixed bottom-24 left-4 right-4 z-50 flex items-center space-x-2"
          >
            <div 
             onClick={() => setIsVisible(true)}
             className="flex-1 pointer-events-auto bg-vibe-bg/80 backdrop-blur-xl border border-vibe-border p-4 rounded-2xl flex items-center justify-between shadow-2xl"
            >
            ...
            <button 
              ...
              className="pointer-events-auto bg-white/5 p-4 rounded-2xl border border-vibe-border text-vibe-muted hover:text-white"
            >
              <X className="w-5 h-5" />
            </button>
          </motion.div>
       ...
       {!isVisible && !installedSuccessfully && deferredPrompt && (
         <motion.div 
           ...
           animate={{ y: 0 }}
           className={cn(
             "fixed z-50 transition-all duration-500 flex items-center space-x-2",
         ...
         >
           <div 
             ...
             className={cn(
               ...
               isMinimized ? "p-3 w-14 h-14 justify-center" : "p-4 flex-1 justify-between"
             )}
           >
             {isMinimized ? (
                <div className="relative">
