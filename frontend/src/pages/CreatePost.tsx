...
import { Camera, X, Image as ImageIcon, Loader2, Sparkles, Send, MapPin, Smile, ChevronRight, ShieldAlert, Users, PlayCircle } from 'lucide-react';
...
export default function CreatePost() {
  ...
  const [postType, setPostType] = useState<'image' | 'text' | 'video' | 'story'>(
    (location.state as any)?.type || 'image'
  );
  ...
  const handleFiles = (files: FileList) => {
    Array.from(files).forEach(file => {
      if (file && file.type.startsWith('image/')) {
        ...
        const reader = new FileReader();
        reader.onload = (e) => {
          setMediaUrls([e.target?.result as string]); // For now, only 1 video per post
          setPostType('video');
        };
        reader.readAsDataURL(file);
      }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Validate
    if ((postType === 'image' || postType === 'video' || postType === 'story') && mediaUrls.length === 0) {
    ...
    try {
      if ((postType === 'image' || postType === 'story') && mediaUrls.length > 0) {
        // Check first image for quick validation, or slice all
        const isImageSafe = await SafetyService.checkImageSafety(mediaUrls[0]);
        if (!isImageSafe) {
      ...
      if (postType === 'story') {
        const storyData = {
          ...
          type: mediaUrls[0].startsWith('data:video') ? 'video' : 'image',
          mediaUrl: mediaUrls[0].trim(),
      ...
      if (postType === 'image' && mediaUrls.length > 0) {
        if (mediaUrls.length === 1) {
          postData.mediaUrl = mediaUrls[0].trim();
        } else {
          postData.mediaUrls = mediaUrls.map(u => u.trim());
          postData.mediaUrl = mediaUrls[0].trim(); // Primary image
        }
  ...
  const canPost = (postType === 'image' || postType === 'video' || postType === 'story') ? (mediaUrls.length > 0 && !isSubmitting) : (caption.trim().length > 0 && !isSubmitting);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-[100] flex items-center justify-center p-0 md:p-6 lg:p-10 gaming-grid overflow-y-auto overflow-x-hidden">
      ...
      >
        ...
        {/* Media / Preview Side */}
        <div className={cn(
          "flex-1 relative flex items-center justify-center p-6 md:p-10 transition-all duration-500 min-h-[400px] md:min-h-0",
          postType === 'text' ? bgColor : 'bg-black'
        )}>
          {postType === 'image' || postType === 'story' ? (mediaUrls.length > 0 ? (
            <div className="relative group w-full h-full flex flex-col items-center justify-center space-y-4">
              <div className="relative w-full h-full flex items-center justify-center">
                <motion.img 
                  ...
                  className={cn(
                    ...
                    postType === 'story' && "h-[90%] aspect-[9/16] object-cover"
                  )} 
                />
                
                {/* Thumbnails list for multiple images */}
                {postType === 'image' && mediaUrls.length > 1 && (
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2 bg-black/40 backdrop-blur-md p-2 rounded-2xl border border-white/10 max-w-[90%] overflow-x-auto no-scrollbar">
                    {mediaUrls.map((url, idx) => (
                      <div key={idx} className="relative group/thumb flex-shrink-0">
                        <img 
                          src={url} 
                          className={cn(
                            "w-12 h-12 rounded-lg object-cover cursor-pointer border-2 transition-all",
                            idx === 0 ? "border-vibe-neon-blue scale-110" : "border-transparent opacity-60 hover:opacity-100"
                          )} 
                          onClick={() => {
                            // Move selected to front
                            const newUrls = [...mediaUrls];
                            const selected = newUrls.splice(idx, 1)[0];
                        ...
                        <button 
                          ...
                          className="absolute -top-1 -right-1 bg-red-500 rounded-full p-0.5 text-white opacity-0 group-hover/thumb:opacity-100 transition-opacity"
                        >
                          <X className="w-3 h-3" />
                        </button>
                      </div>
              ...
              {postType === 'image' && (
                <button 
                  ...
                  className="flex items-center space-x-2 text-vibe-neon-blue font-black uppercase text-[10px] tracking-widest bg-vibe-neon-blue/10 px-4 py-2 rounded-full hover:bg-vibe-neon-blue/20 transition-all"
                >
            ...
            >
              ...
              <h3 className="text-lg font-black text-vibe-text uppercase tracking-widest">
                {postType === 'story' ? 'Novo Story' : 'Post de Imagens'}
              </h3>
              <p className="text-vibe-muted text-xs font-bold uppercase mt-2">Clique ou arraste arquivos (múltiplos ok)</p>
              
              <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/*,video/*" multiple={postType === 'image'} />
            </div>
          )) : postType === 'video' && mediaUrls.length > 0 ? (
            <div className="relative group w-full h-full flex items-center justify-center bg-black rounded-[32px] overflow-hidden">
              <motion.video 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                src={mediaUrls[0]} 
              ...
              <button 
                onClick={() => removeMedia(0)}
                className="absolute top-6 right-6 bg-red-500 p-3 rounded-full text-white shadow-xl hover:scale-110 active:scale-95 transition-all z-20"
              >
                <X className="w-6 h-6" />
              </button>
              <div className="absolute top-6 left-6 bg-vibe-neon-blue text-vibe-bg px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-glow-blue flex items-center space-x-2">
            ...
            >
              <div className="w-20 h-20 bg-vibe-card rounded-2xl flex items-center justify-center text-vibe-muted mb-6 group-hover:text-vibe-neon-blue group-hover:scale-110 transition-all shadow-xl">
                 {postType === 'video' ? <PlayCircle className="w-10 h-10" /> : <ImageIcon className="w-10 h-10" />}
              </div>
              <h3 className="text-lg font-black text-vibe-text uppercase tracking-widest">Novo Reel</h3>
              <p className="text-vibe-muted text-xs font-bold uppercase mt-2">Clique ou arraste um arquivo</p>
              
              <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="video/*" multiple={false} />
            </div>
          )}
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex bg-black/40 backdrop-blur-xl p-1 rounded-2xl border border-white/10">
            <button 
              ...
              className={cn(
                ...
                postType === 'story' ? "bg-vibe-neon-blue text-vibe-bg shadow-lg shadow-vibe-neon-blue/20" : "text-white/40 hover:text-white"
              )}
            >
              Story
            </button>
            ...
            <button 
              ...
              className={cn(
                "px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all",
                postType === 'video' ? "bg-vibe-neon-blue text-vibe-bg shadow-lg shadow-vibe-neon-blue/20" : "text-white/40 hover:text-white"
              )}
            >
              Vídeo
            </button>
