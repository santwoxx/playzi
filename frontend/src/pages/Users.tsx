...
export default function Users() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const usersData = snapshot.docs.map(doc => {
        ...
        return uNickname !== 'anaclara0927bb' && uDisplayName !== 'anaclara0927bb' && uId !== 'anaclara0927bb';
      });
      setUsers(usersData);
      setLoading(false);
    });
  ...
  const performSearch = async (term: string) => {
    ...
    try {
      ...
      const unique = Array.from(new Map(merged.map(u => [u.uid, u])).values()).filter(u => {
        const uNickname = (u.nickname || '').toLowerCase();
        const uDisplayName = (u.displayName || '').toLowerCase();
        const uId = (u.uid || '').toLowerCase();
        return uNickname !== 'anaclara0927bb' && uDisplayName !== 'anaclara0927bb' && uId !== 'anaclara0927bb';
  ...
  return (
    <div className="pb-nav pt-4 px-4 max-w-2xl mx-auto space-y-6">
      <SEO 
        title="Encontrar Amigos e Matchmaking Gamer" 
        description="Encontre gamers compatíveis com seu estilo de jogo. Use nosso matchmaking para achar o duo perfeito, novos amigos para chat e squads de elite." 
        keywords="encontrar duo, matchmaking gamer brasil, amigos para jogar online, buscar jogadores"
      />
