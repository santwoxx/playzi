            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {loading ? (
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="animate-pulse">{isRegister ? 'Criando sua Lenda...' : 'Conectando à Arena...'}</span>
                  </div>
                ) : (
                  <>
                    {isRegister ? <UserPlus className="w-4 h-4" /> : <LogIn className="w-4 h-4" />}
                    <span>{isRegister ? 'Forjar Conta' : 'Entrar na Arena'}</span>
                  </>
                )}
              </button>
              ...
              {!isRegister && (
                <button 
                  ...
                  className="w-full bg-white/5 text-vibe-muted font-bold py-3 px-6 rounded-2xl flex items-center justify-center space-y-1 md:space-y-0 md:space-x-2 active:scale-95 disabled:opacity-50 transition-all text-[10px] uppercase tracking-widest border border-white/5"
                >
                  {loadingAnon ? (
                    <div className="flex items-center space-x-2">
                      <div className="w-4 h-4 border-2 border-vibe-muted/30 border-t-vibe-muted rounded-full animate-spin" />
                      <span className="animate-pulse">Entrando como Espectador...</span>
                    </div>
                  ) : (
                    <>
                      <UserIcon className="w-4 h-4" />
                      <span>Jogar sem Conta</span>
                    </>
                  )}
                </button>
              )}
