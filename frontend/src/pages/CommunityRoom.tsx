...
import { MessageSquare, Mic, Users, Send, PhoneOff, MicOff, Volume2, Gamepad2, Trophy, Hammer, ShieldAlert, Plus, MessageCircle } from 'lucide-react';
...
export default function CommunityRoom() {
  ...
  const isOfficial = COMMUNITY_DATA[categoryId || ''];
  const data = isOfficial || community || { name: 'Carregando...', logo: '', rooms: [] };

  useEffect(() => {
    if (!categoryId) return;
    
    // If it's a dynamic community (not official), fetch it
    if (!isOfficial) {
      const communityRef = doc(db, 'communities', categoryId);
  ...
  const handleJoin = async () => {
    if (!currentUser || !categoryId || isOfficial) return;
    try {
      const communityRef = doc(db, 'communities', categoryId);
  ...
  const handleInvite = () => {
    const inviteLink = `${window.location.origin}/communities/${categoryId}`;
    ...
    <div className="pt-16 h-screen flex flex-col bg-vibe-bg">
      ...
      <div className="bg-black/40 border-b border-vibe-border px-6 py-4 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className="w-12 h-12 rounded-xl bg-vibe-bg border border-vibe-border p-1.5 shadow-[0_0_15px_rgba(0,242,255,0.1)] overflow-hidden">
            <img src={data.logo || data.icon} className="w-full h-full object-contain" referrerPolicy="no-referrer" />
          </div>
          <div>
            ...
            <div className="flex items-center text-[10px] font-black uppercase tracking-widest text-vibe-muted">
              ...
              {t('online_players_count', { count: data.members?.length || 1248 })}
            </div>
          </div>
        </div>
        <div className="flex space-x-2">
           {!isOfficial && currentUser && !data.members?.includes(currentUser.uid) && (
             <button 
              onClick={handleJoin}
              className="px-4 py-2 bg-vibe-neon-blue text-vibe-bg rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center shadow-glow-blue"
             >
               Participar
             </button>
           )}
           <button 
             ...
             className="p-2 bg-white/5 rounded-xl hover:text-vibe-neon-blue transition-colors group"
      ...
      <div className="flex-1 overflow-hidden relative">
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'members' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full p-6 overflow-y-auto no-scrollbar space-y-6">
               <div className="flex items-center justify-between">
                 <h3 className="text-xs font-black uppercase tracking-widest text-vibe-muted">Membros do Squad ({data.members?.length || 0})</h3>
               </div>
               
               <div className="space-y-3">
                 {data.members?.map((memberId: string) => (
                   <MemberItem key={memberId} userId={memberId} isOwner={data.ownerId === memberId} />
                 ))}
                 
                 {isOfficial && [1,2,3,4,5].map(i => (
                   <div key={i} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5">
                      <div className="flex items-center space-x-4">
                        <img src={`https://picsum.photos/seed/mem${i}/40/40`} className="w-10 h-10 rounded-full" />
                        <div>
                          <p className="text-sm font-black text-white">Gamer_{i}</p>
                          <p className="text-[10px] text-vibe-muted font-bold uppercase tracking-widest">Nível {10 + i}</p>
                        </div>
                      </div>
                   </div>
                 ))}
...
function MemberItem({ userId, isOwner }: { userId: string, isOwner: boolean }) {
  ...
  if (!user) return <div className="h-16 animate-pulse bg-white/5 rounded-2xl" />;

  return (
    <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-vibe-neon-blue/30 transition-all group">
      <div className="flex items-center space-x-4">
        <div className="relative">
          <img 
            src={user.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.nickname || 'U')}`} 
            className="w-10 h-10 rounded-full border border-vibe-border" 
          />
          {isOwner && (
            <div className="absolute -top-1 -right-1 bg-yellow-500 rounded-full p-0.5 border border-vibe-bg">
              <Trophy className="w-2.5 h-2.5 text-vibe-bg" />
            </div>
          )}
        </div>
        <div>
          <h4 className="text-sm font-black text-white">{user.nickname}</h4>
          <p className="text-[10px] text-vibe-muted font-bold uppercase tracking-widest">Nível {user.level || 1}</p>
        </div>
      </div>
      <button 
        onClick={() => navigate(`/chat/${userId}`)}
        className="p-2 bg-white/5 rounded-xl text-vibe-muted hover:text-vibe-neon-blue transition-colors"
      >
